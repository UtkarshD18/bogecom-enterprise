...
public class CartService {
    ...
    public CartDto updateCartItem(Long userId, Long itemId, UpdateCartItemRequest request) {
        ...
        CartItem itemToUpdate = cart.getItems().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new BusinessException("Item not found in cart", HttpStatus.NOT_FOUND));
    ...
    public void mergeGuestCartWithUserCart(Long userId, String sessionId) {
        ...
        Optional<Cart> guestCartOpt = cartRepository.findBySessionIdAndStatusAndIsDeletedFalse(sessionId, CartStatus.ACTIVE);
    ...
    private Cart getOrCreateCartEntity(Long userId, String sessionId) {
        if (userId != null) {
            return cartRepository.findByUserIdAndStatusAndIsDeletedFalse(userId, CartStatus.ACTIVE)
                    .orElseGet(() -> createNewCart(userId, null));
        } else if (sessionId != null) {
            return cartRepository.findBySessionIdAndStatusAndIsDeletedFalse(sessionId, CartStatus.ACTIVE)
                    .orElseGet(() -> createNewCart(null, sessionId));
        } else {
            throw new BusinessException("Either userId or sessionId must be provided", HttpStatus.BAD_REQUEST);
        }
    }
