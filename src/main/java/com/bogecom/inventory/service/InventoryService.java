...
public class InventoryService {
  ...
  public InventoryDto adjustInventory(AdjustInventoryRequest request) {
    ...
    Inventory inventory =
        inventoryRepository
            .findByProductIdAndLocationAndIsDeletedFalse(request.productId(), request.location())
            .orElseGet(
                () -> {
    ...
    log.info(
        "Adjusted inventory for product {} at {} to {}",
        request.productId(),
        request.location(),
  ...
  public InventoryReservationDto reserveInventory(ReserveInventoryRequest request) {
    if (request.cartId() == null && request.orderId() == null) {
      throw new BusinessException(
          "Either cartId or orderId must be provided", HttpStatus.BAD_REQUEST);
    }
    ...
    List<Inventory> inventories =
        inventoryRepository.findByProductIdAndIsDeletedFalse(request.productId());
    ...
    Inventory selectedInventory =
        inventories.stream()
            ...
            .findFirst()
            .orElseThrow(
                () -> new BusinessException("Insufficient stock available", HttpStatus.CONFLICT));

    // Adjust quantities
    selectedInventory.setAvailableQuantity(
        selectedInventory.getAvailableQuantity() - request.quantity());
    selectedInventory.setReservedQuantity(
        selectedInventory.getReservedQuantity() + request.quantity());
  ...
  private void syncProductStockQuantity(Product product) {
    List<Inventory> inventories =
        inventoryRepository.findByProductIdAndIsDeletedFalse(product.getId());
